...
def attention_3d_block2(inputs, single_attention_vector: bool = False):
    ...
    步骤：
      ...
      2. Dense(time_steps, tanh)：对每个特征在所有时间步上学习注意力得分
      3. Softmax（在 Activation 层中）：将得分归一化为概率权重
      4. Permute((2,1))：再交换回来，恢复原始维度顺序
      5. Multiply：将注意力权重与原始输入逐元素相乘
...
def build_attention_model(
    ...
    dropout_rate : Conv1D 和 BiLSTM 后的 Dropout 比例，默认 0.1（10%）。
    ...
    # attention_3d_block2 是默认模式（TF 2.x 兼容，"permute" 模式）
